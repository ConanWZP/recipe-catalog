...
import IngrStoreItem from "../components/IngrStoreItem/IngrStoreItem";
import {
    getRecipesByIngredients,
    setCurrentPageSearchByIngr,
    setSearchString
} from "../redux/reducers/searchByIngrSlice";

const IngredientStorePage = () => {
    const dispatch = useAppDispatch()
    const {items} = useAppSelector(state => state.ingredientsStore)
    const {dishesByIngrObj} = useAppSelector(state => state.searchByIngr)

    let arrayItemsName = [];
    arrayItemsName.push(items.map(item => item.name))
    let stringItemsName = arrayItemsName.join()
    let replacedStringItemsName = stringItemsName.replaceAll(',', ',+')
    ...
    useEffect(() => {
        if (replacedStringItemsName) {
            setVisibleModal(true)
            dispatch(setCurrentPageSearchByIngr(1))
            dispatch(setSearchString(replacedStringItemsName))
        }
        if (items.length === 0) {
            setVisibleModal(false)
        }
    }, [replacedStringItemsName, items])

    const onClickGetRecipeByIngrList = () => {
        if (replacedStringItemsName) {
        ...
        <div className={`${styles.storeWrapper}  ${styles.storeWrapperStore}`}>
            <div className="cart">
                <div className="cart__top">
                    ...
                    <div className="cart__clear" onClick={() => dispatch(clearItemStore())}>
                ...
                <div className="content__items">
                    ...
                    {
                        items.map(i =>
                            <IngrStoreItem name={i.name} image={i.image} id={i.id} key={i.id}/>
                        )
                    }
                ...
                <div className="cart__bottom">
                    <div className="cart__bottom-details">
                        <span> Total Ingredients: <b>{items.length > 0 ? items.length : 0}</b> </span>
                            ...
                            visibleModal ?
                                <NavLink to={'/recipesByIngredients'}>
                                    <div className={"button pay-btn" + ' ' + styles.buttonSize}
                                         onClick={onClickGetRecipeByIngrList}>
...
export default IngredientStorePage;